...
local function processData(dataFromPbString)
	if dataFromPbString and dataFromPbString~="" then
		local dataFromPb = split(dataFromPbString,",")
				
		local deltaTime = getDeltaTime()
		
		if initialTime ~= dataFromPb[1] then
			initialTime = dataFromPb[1]
